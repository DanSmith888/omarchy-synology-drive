...
- **No pill width reserve, by Daniel's call (2026-09-01).** The skill's
  grow-only `reservedWidth` rule is for pills whose reading changes every
  poll; this one only changes with the sync state. 0.2.3 had the reserve,
  0.2.4 removed it — don't re-add.
